...
const AdminCinemas = () => {
  ...
  const handleCinemaSubmit = async (e) => {
    ...
    try {
      ...
      toast.error("Gagal menambah bioskop: " + (error.response?.data?.message || error.message));
    } finally {
  ...
  const handleDeleteCinema = async (id, name) => {
    if (window.confirm(`Hapus bioskop "${name}" beserta seluruh studionya?`)) {
      try {
        ...
        toast.error("Gagal menghapus bioskop");
      }
    }
  };
  ...
  const handleStudioSubmit = async (e) => {
    ...
    try {
      ...
      toast.error("Gagal menambah studio: " + (error.response?.data?.message || error.message));
    } finally {
      setSubmitLoading(false);
    }
  };
            ...
            <div className="p-8">
              <form id="studioForm" onSubmit={handleStudioSubmit} className="space-y-6">
                ...
                <div className="bg-sky-500/10 border border-sky-500/20 p-4 rounded-xl flex items-start gap-3">
                  ...
                  <p className="text-xs text-sky-400/80 font-medium leading-relaxed">
