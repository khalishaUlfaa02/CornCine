...
const AdminSchedules = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (schedRes.success) {
        const schedList =
          schedRes.data?.content || schedRes.data?.data || (Array.isArray(schedRes.data) ? schedRes.data : []);
        setSchedules(schedList);
      }

      if (movieRes.success) {
        const movieList =
          movieRes.data?.content || movieRes.data?.data || (Array.isArray(movieRes.data) ? movieRes.data : []);
        setMovies(movieList);
      }

      if (cinemaRes.success) {
        const cinemaList =
          cinemaRes.data?.content || cinemaRes.data?.data || (Array.isArray(cinemaRes.data) ? cinemaRes.data : []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error('Gagal menambah jadwal: ' + (error.response?.data?.message || error.message));
    } finally {
                    ...
                    <tr key={sched.scheduleId} className="hover:bg-slate-800/30 transition-colors">
                      ...
                      <td className="py-4 px-6">
                        ...
                        <p className="text-xs text-slate-500">
                          Studio {sched.studioNumber} ({sched.studioType})
                        </p>
                      </td>
                      <td className="py-4 px-6">
          ...
          <div className="bg-slate-900 border border-slate-700/80 rounded-3xl w-full max-w-xl shadow-2xl relative z-10 flex flex-col">
                    ...
                    >
                      ...
                      {studios.map((s) => (
                        <option key={s.studioId} value={s.studioId}>
                          Studio {s.studioNumber} ({s.studioType})
                        </option>
                      ))}
                    </select>
                  </div>
            ...
            <div className="p-6 border-t border-slate-800 flex justify-end gap-3 shrink-0 bg-slate-900/50 rounded-b-3xl">
              ...
              <button
                ...
                className={`bg-sky-400 text-slate-950 font-bold px-6 py-2.5 rounded-xl transition-all duration-300 shadow-[0_0_15px_rgba(56,189,248,0.3)] hover:shadow-[0_0_20px_rgba(56,189,248,0.5)] transform hover:-translate-y-0.5 ${
