...
import { getMovies, createMovie, updateMovie, deleteMovie, getGenres, createGenre, uploadFile, getStoredToken, exportMoviesToExcel, importMoviesFromExcel } from '../../api/movieApi';
...
const AdminMovies = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const finalGenreIds = [];
      let currentServerGenres = [...genres];
      ...
      if (finalGenreIds.length === 0) {
        toast.error("Gagal memetakan genre. Pastikan minimal ada 1 genre yang valid.");
      ...
      toast.error("Gagal menyimpan data film: " + (error.response?.data?.message || error.message || "Error tidak diketahui"));
      console.error(error);
    } finally {
      setSubmitLoading(false);
      setUploadingPoster(false);
  ...
  const handleExport = async () => {
    try {
      const blob = await exportMoviesToExcel();
      const url = window.URL.createObjectURL(new Blob([blob]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', 'Daftar_Film_CornCine.xlsx');
      document.body.appendChild(link);
      link.click();
      link.parentNode.removeChild(link);
      window.URL.revokeObjectURL(url);
    } catch (error) {
      toast.error("Gagal mengekspor data film ke Excel.");
    }
  };
  ...
  const handleImport = async (e) => {
    ...
    const confirmImport = window.confirm(`Import data film dari file "${file.name}"?`);
    if (!confirmImport) {
      e.target.value = '';
      return;
    }
    ...
    try {
      ...
      await importMoviesFromExcel(formData);
      toast.success("Data film berhasil diimpor dari Excel!");
      fetchData();
    } catch (error) {
      toast.error("Gagal mengimpor data: " + (error.response?.data?.message || "Pastikan format file sesuai template Export."));
    } finally {
      e.target.value = '';
    }
  };
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <button 
              ...
              className="bg-slate-800 hover:bg-slate-700 text-emerald-400 font-bold px-5 py-3 rounded-xl transition-all duration-300 border border-slate-700 hover:border-emerald-500/50 whitespace-nowrap"
            >
              ⬇ Export Excel
            </button>
            <label className="bg-slate-800 hover:bg-slate-700 text-amber-400 font-bold px-5 py-3 rounded-xl transition-all duration-300 border border-slate-700 hover:border-amber-500/50 cursor-pointer whitespace-nowrap">
              ⬆ Import Excel
                  ...
                  movies.map((movie) => (
                    <tr key={movie.movieId} className="hover:bg-slate-800/30 transition-colors">
                      ...
                      <td className="py-4 px-6">
                        ...
                        <p className="text-xs text-slate-400 truncate max-w-xs">{Array.isArray(movie.genres) ? movie.genres.join(', ') : movie.genres}</p>
                      </td>
                      <td className="py-4 px-6">
