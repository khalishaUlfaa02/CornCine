...
const ForgotPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccess(response.message || 'Token reset password telah dikirim ke email Anda.');
    } catch (err) {
  ...
  return (
    <div className="min-h-screen pt-32 pb-16 flex items-center justify-center px-4 relative z-10">
      <div className="bg-cine-card p-10 rounded-2xl shadow-2xl w-full max-w-md border border-cine-border relative overflow-hidden">
        ...
        <h2 className="text-3xl font-extrabold text-center text-white mb-2">Lupa Password</h2>
        <p className="text-center text-cine-muted mb-8 font-medium">
          Masukkan email akun Anda, kami akan mengirim token reset password.
        </p>
        ...
        <div className="mt-8 text-center">
          <p className="text-cine-muted text-sm font-medium">
            Sudah dapat token?{' '}
            <Link to="/reset-password" className="text-cine-baby hover:text-cine-baby-hover font-bold transition-colors">
              Reset password
            </Link>
          </p>
